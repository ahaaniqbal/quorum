...
const LOOP = [
  {
    ...
    icon: Inbox,
    label: "Ingest",
    body: "New inbound, a CSV, or a webhook lands in one queue. Every account starts the same way.",
  },
  {
    n: "02",
    icon: BrainCircuit,
    label: "Build the brain",
    body: "Quorum researches the company, identifies committee gaps, and decides the next best move.",
  },
  {
    n: "03",
    icon: ShieldCheck,
    label: "Review",
    body: "Drafts and risky actions wait behind a human gate before anything is customer-facing.",
  },
  {
    n: "04",
    icon: Send,
    label: "Close the loop",
    body: "Approved work becomes CRM updates, messages, meetings, and team alerts with receipts.",
  },
];

const PROBLEMS = [
  {
    title: "Speed to lead dies in handoffs",
    body: "A lead arrives, gets routed, sits in a queue, and the moment is gone before a human replies. The account cools while the work waits.",
  },
  {
    title: "Sequencers think in contacts",
    body: "Engagement tools blast one inbox at a time. Real deals close across a buying committee, with memory of every prior touch.",
  },
  {
    title: "AI without governance is dangerous",
    body: "Autonomous outreach with no review, no source of truth, and no receipts puts your brand and your pipeline at risk.",
  },
];
...
const CHANNELS = [
  {
    ...
    body: "Qualified accounts, risky actions, and approval-needed nudges post straight into your revenue channel.",
  },
  {
    key: "crm",
    label: "CRM",
    icon: Database,
    body: "Accounts, contacts, notes, and deals are created and kept in sync in HubSpot or Salesforce after review.",
  },
  {
    key: "email",
    label: "Email",
    icon: MailCheck,
    body: "Persona-tuned outreach to the whole committee, sent from the seller's mailbox once a human approves it.",
  },
  {
    key: "calendar",
    label: "Calendar",
    icon: CalendarCheck,
    body: "Meetings get booked and follow-ups scheduled the moment a prospect agrees, with the right context attached.",
  },
] as const;

const MOAT = [
  {
    title: "Why not just use a CRM copilot?",
    body: "A copilot answers questions about one record. Quorum works the entire account: committee, memory, drafts, the next move, and the action itself.",
  },
  {
    title: "Why not a sales engagement tool?",
    body: "Sequencers automate messages. Quorum reasons about accounts, committee coverage, timing, and the next safe action, with a human gate on every send.",
  },
  {
    title: "Why now?",
    body: "Inbound volume outpaces headcount, and models are finally good enough to do real GTM work, as long as every action stays governed and auditable.",
  },
];
...
function Hero() {
  return (
    <section className="relative mx-auto max-w-[1180px] px-6 pb-20 pt-20 sm:pt-28">
      {/* orange dawn glow behind the headline */}
      <div
        aria-hidden
        className="pointer-events-none absolute left-1/2 top-0 h-[420px] w-[820px] max-w-[120vw] -translate-x-1/2 opacity-60"
        style={{
          background:
            "radial-gradient(60% 100% at 50% 0%, rgba(249,115,22,0.28), rgba(249,115,22,0.05) 45%, transparent 70%)",
        }}
      />
      <div className="relative text-center">
        <Reveal>
          <div className="mx-auto mb-6 flex h-9 w-9 items-center justify-center border border-accent-subtle bg-surface">
            <Sparkles size={16} strokeWidth={2} className="text-accent" />
          </div>
          <p className="mono-label text-accent-soft">AI account execution</p>
          <h1 className="mx-auto mt-5 max-w-4xl text-balance text-[40px] font-semibold leading-[1.04] tracking-tight sm:text-[68px]">
            <span className="text-secondary">It replaces the </span>
            messy first 48 hours
            <span className="text-secondary"> after a qualified account appears.</span>
          </h1>
          <p className="mx-auto mt-6 max-w-2xl text-pretty text-[16px] leading-relaxed text-secondary sm:text-[18px]">
            Quorum is not a demo chatbot. It is an execution layer with connectors, approval gates,
      ...
      {/* Operating loop preview */}
      <Reveal delay={0.1} className="relative mt-16">
        <div className="cell overflow-hidden">
          <div className="flex items-center justify-between border-b border-border px-4 py-3">
            <span className="mono-label text-tertiary">Operating loop</span>
            <span className="mono-label tnum text-accent-soft">live</span>
          </div>
          <div className="grid grid-cols-1 divide-y divide-border md:grid-cols-4 md:divide-x md:divide-y-0">
            {LOOP.map((step) => {
              const Icon = step.icon;
              return (
                <div key={step.n} className="group relative p-5">
                  <div className="flex items-center justify-between">
                    <span className="flex h-9 w-9 items-center justify-center border border-accent-subtle text-accent transition-colors group-hover:border-accent">
                      <Icon size={16} strokeWidth={2} />
                    </span>
                    <span className="mono-label tnum text-tertiary">{step.n}</span>
                  </div>
                  <h3 className="mt-4 text-[15px] font-semibold text-text">{step.label}</h3>
...
function Problem() {
  return (
    <section className="relative border-t border-border bg-bg/60 py-24">
                    ...
                    <span className="min-w-0">
                      ...
                      <span
                        className={`mt-1 block overflow-hidden text-[13px] leading-relaxed text-secondary transition-all ${
                          on ? "max-h-24 opacity-100" : "max-h-0 opacity-0 lg:max-h-0"
                        }`}
                      >
                        {c.body}
                      </span>
  ...
  return (
    <section className="relative">
      <div className="relative mx-auto max-w-[1180px] px-6">
        <Reveal>
          <div className="relative overflow-hidden bg-accent px-8 py-16 text-center sm:py-20">
            <div className="relative">
              ...
              <div className="mt-8 flex flex-col items-center justify-center gap-3 sm:flex-row">
                ...
                <a
                  href={authUrl("/signup")}
                  className="inline-flex h-12 items-center gap-2 border border-black/30 px-6 text-[14px] font-medium text-black transition-colors hover:border-black"
                >
                  Create workspace
                </a>
              </div>
  ...
  return (
    <footer className="relative overflow-hidden border-t border-border pt-16">
      <div className="mx-auto max-w-[1180px] px-6">
        <div className="flex flex-col items-start justify-between gap-6 pb-10 sm:flex-row sm:items-center">
          ...
          <div className="flex items-center gap-5 text-[13px] text-secondary">
            <a href={authUrl("/signin")} className="transition-colors hover:text-text">
              Sign in
            </a>
            <a href={authUrl("/signup")} className="transition-colors hover:text-text">
              Create workspace
            </a>
          </div>
        </div>
      ...
      <div className="pointer-events-none select-none px-4">
        <p className="-mb-[0.18em] text-center font-semibold leading-none tracking-tighter text-text/[0.07] text-[26vw]">
          quorum
        </p>
      </div>
    </footer>
